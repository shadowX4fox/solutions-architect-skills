...
2. **Otherwise**: build `scanned_text` by concatenating, in order:
   a. `**Type:**`, `**Technology:**`, `**Description:**`, `**Communicates via:**`, `**Deploys as:**` (the canonical 5).
   b. The body of the `<!-- EXPLORER_HEADER ... -->` block if present (specifically the `technologies:` line and the `key_concepts:` line).
   c. Every row of every table that follows an H2/H3 heading matching (case-insensitive) `Subscriptions`, `Subscribes To`, `Produces`, `Consumes`, `Topics`, `Streams`, or `Events` — concatenate the cell contents only (no header rows).
   Then keyword-match against the vocabulary in PAYLOAD_SCHEMA.md. The `**Type:**` row that matches first determines `component_type` (the broader scan does not influence component_type, only asset_types). Each vocabulary row whose keywords match anywhere in `scanned_text` contributes its entries to the union of `asset_types`. Append `c4-descriptor` (always-on except for `skip`). Deduplicate. Apply the messaging-row's avro/protobuf sub-rule by re-scanning `scanned_text` for the literal tokens `avro|*.avsc|Schema Registry` (with the qualifier that `Schema Registry` only adds `avro` if `avro` is also present in `scanned_text`) and `protobuf|proto3|*.proto|gRPC`.
