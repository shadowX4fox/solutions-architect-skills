...
This skill acts as a **compliance portfolio health reviewer**. It validates that all 10 compliance contracts are present and fresh (≤6 months old), analyzes each contract's requirement gaps, and generates an interactive HTML playground for exploring the concepts in ARCHITECTURE.md that need to be fixed to reach the auto-approve threshold (≥8.0/10) across all contracts.
...
- `architectureSection`: inferred ARCHITECTURE.md section(s) to update (based on source sections from gap rows)
- `fixGuidance`: specific, actionable guidance on what to document in ARCHITECTURE.md
  ...
  conceptClusters: [
    {
      ...
      architectureSection: "<section(s) to update>",
      fixGuidance: "<specific actionable guidance>"
    }
  ]
}
```
...
- Identify the highest-impact concept clusters to fix first
- See which ARCHITECTURE.md sections need updating
- Understand which contracts are below the auto-approve threshold
- Generate a targeted fix prompt for updating ARCHITECTURE.md
- Paste the fix prompt back into Claude with `/skill architecture-docs` to apply changes
...
| `architecture-docs` | Downstream fix target: concept cluster fix prompts generated here should be applied using the architecture-docs skill's context-efficient editing workflow. |
...
- [ ] Fix prompts reference specific ARCHITECTURE.md sections (not generic advice)
