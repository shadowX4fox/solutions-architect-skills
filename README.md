...
| CC Code | Contract Type | Focus | Requirements |
|---------|--------------|-------|-------------|
| CC-001 | Business Continuity | RTO/RPO, disaster recovery, backup, resilience | 43 LACN |
| CC-002 | Cloud Architecture | Deployment model, provider, connectivity, security | 6 LAC |
| CC-003 | Data & AI Architecture | Data quality, AI governance, model validation | 11 LAD/LAIA |
| CC-004 | Development Architecture | Technology stack, coding standards, 26-item validation | 2 LADES |
| CC-005 | Enterprise Architecture | Strategic alignment, modularity, cloud-first, governance | 7 LAE |
| CC-006 | Integration Architecture | Integration catalog, patterns, API security | 7 LAI |
| CC-007 | Platform & IT Infrastructure | Environments, databases, capacity, naming | 9 LAPI |
| CC-008 | Process Transformation | Automation, efficiency, ROI analysis | 4 LAA |
| CC-009 | Security Architecture | API security, authentication, encryption, controls | 8 LAS |
| CC-010 | SRE Architecture | SLOs, error budgets, monitoring, incident management | 57 LASRE |

#### Compliance Generation Architecture

Each compliance contract is generated by a **compliance generator agent** that delegates external validation to a dedicated **validator agent**:
...
```
                    ...
                    │       (orchestrator)         │
                    └──────────┬──────────────────┘
                               │ spawns 10 in parallel
                 ┌─────────────┼─────────────┐
                 ▼             ▼             ▼
    ┌────────────────┐ ┌────────────────┐ ┌────────────────┐
    │ CC-001 Generator│ │ CC-004 Generator│ │ CC-010 Generator│
    │ (business-cont.)│ │ (development)   │ │ (sre)           │  ... x10
    └───────┬────────┘ └───────┬────────┘ └───────┬────────┘
            │                  │                  │
            │ Step 3.4         │ Step 3.4         │ Step 3.4
            ▼                  ▼                  ▼
    ┌────────────────┐ ┌────────────────┐ ┌────────────────┐
    │ CC-001 Validator│ │ CC-004 Validator│ │ CC-010 Validator│
    │  (16 items)     │ │  (26 items +   │ │  (25 items)     │  ... x10
    │                 │ │   EOL checks)  │ │                 │
    └────────────────┘ └────────────────┘ └────────────────┘
            │                  │                  │
            ▼                  ▼                  ▼
    ...
    ┌─────────────────────────────────────────────────────┐
    │              compliance-docs/                        │
    │  CC-001-business-continuity_Project_2026-03-28.md   │
    │  CC-004-development-architecture_Project_2026-03-28.md │
    │  CC-010-sre-architecture_Project_2026-03-28.md      │
    │  ...                                                 │
    │  COMPLIANCE_MANIFEST.md                              │
    └─────────────────────────────────────────────────────┘
```

#### Example: CC-004 Development Architecture Flow

The Development contract (CC-004) demonstrates the full validation pattern:

```
CC-004 Generator                          CC-004 Validator (Hephaestus)
─────────────────                         ────────────────────────────────
Phase 1: Load template                    Phase 1: Stack Validation (26 items)
Phase 2: Extract project info             ├─ Java Backend (DEV-01 to DEV-06)
Phase 3: Read docs/06, docs/09, adr/     ├─ .NET Backend (DEV-07 to DEV-12)
...
Step 4.6: Map VALIDATION_RESULT
  ├─ DEV-01→[JAVA_ITEM_1]
  ├─ DEV-07→[DOTNET_ITEM_1]
  ├─ ...→ all 26 placeholders
  ├─ Counts → [PASS_COUNT], [FAIL_COUNT]
  └─ EOL warnings → advisory note
Phase 5: Write CC-004-development-
  architecture_Project_2026-03-28.md
```

The validator is the **sole source of truth** for all external checks. The generator never re-evaluates items — it only maps `VALIDATION_RESULT` fields to template placeholders.
