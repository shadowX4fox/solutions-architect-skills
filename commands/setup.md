...
- Merges four top-level sections into the user file:
  ...
  - `hooks` — event-keyed merge with idempotent recognition of the active sa-skills hook. **Platform-aware command resolution (v3.21.1+)**: the helper detects `process.platform` at install time and writes the absolute path to the matching native wrapper into your settings.json — `route-architecture-docs.sh` on Linux/macOS/WSL/Git Bash, `route-architecture-docs.cmd` on Windows native cmd.exe, or `route-architecture-docs.ps1` for PowerShell. The example file's literal hook command is documentation only; the actual command written to your settings.json is rewritten to the platform-resolved form before the merge runs. Install markers match any of the three wrapper basenames so a Linux user with the .sh entry, a Windows user with the .cmd entry, and a PowerShell user with the .ps1 entry are all recognized as "already installed" — no duplication on re-runs. **Retirement pass (v3.19.1+)**: any user-side hook entry whose command matches `SA_SKILLS_HOOK_REMOVAL_MARKERS` is stripped from the merged `hooks` block and reported under a separate "retired" counter. Currently retired (v3.21.1): `header-cli.ts session-log add` (v3.14.1 PostToolUse editlog tracker), the v3.19.0 `sh ~/.../route-architecture-docs.sh` form, and the v3.21.0 `bun ~/.../route-architecture-docs.ts` form — all three depended on `~` expansion that fails on Windows native (cmd / PowerShell). Compound AND-substring markers (`["sh ~", "route-architecture-docs.sh"]`) distinguish the legacy `~`-prefixed forms from the v3.21.1+ absolute-path wrappers without false-positives. Idempotent — once retired, a second run is a no-op. The helper output now begins with `Platform:` and `Hook command:` lines so you can verify the right wrapper was selected before reading the merge counters.
